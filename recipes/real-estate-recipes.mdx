...
Use `https://backend.tensorpix.ai/api/v2/real-estate` as the base URL. Use `application/json` for JSON data. Use `multipart/form-data` for image files. The API only returns data owned by the account for the API token.
